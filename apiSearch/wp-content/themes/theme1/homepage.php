...
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
  ...
  <script>
    ...
    document.addEventListener('click', function(e) {
        // Проверяем, что кликнули именно по кнопке отправки
    ...
    // Надёжная проверка: ищем форму по наличию поля action или классу
    if (form.matches('form.order-form') && form.querySelector('[name="action"][value="submit_component_request"]')) {
        ...
        if (actionUrl && actionUrl.charAt(0) !== '/') {
            actionUrl = '/' + actionUrl; // делаем абсолютным от корня
        }
        if (!actionUrl) {
    ...
    function waitForJQuery() {
        ...
        jQuery(function($) {
            ...
            const nonce = '<?php echo wp_create_nonce("product_search_nonce"); ?>';
            ...
            function clearSearchErrors() {
                ...
                $('.search__error').remove(); // ← два подчёркивания!
            
                // Убираем CSS-классы ошибок
            ...
            searchModeList.on('change', function() {
                if (searchModeList.is(':checked')) {
                    // Режим списка - показываем textarea
                    clearSearchErrors(); // ← добавь это
           ...
           function search() {
                ...
                if (isSingleMode) {
                    ...
                    const quantity = singleSearchInputQuanity.val().trim() || '1'; // если пусто — 1
                    ...
                    query = article + ':' + quantity; // ← формат "L7912CV:100"
                ...
                if (isSingleMode) {
                    // Отправляем строку вида "артикул:количество"
                    $.post(ajaxurl, {
                        action: 'product_search_by_article',
                        article: query, // ← уже содержит и артикул, и количество
                        security: nonce
                    }, function(response) {
                        clearSearchErrors();
           ...
           // Обработка отправки формы с валидацией
            searchForm.on('submit', function(e) {
                ...
                // Если валидация провалена — ничего не делаем (ошибки уже показаны)
            });
            ...
            function showSearchError(field, message) {
                ...
                // Создаём и вставляем новую ошибку
                const $error = $('<span class="search__error">' + message + '</span>');
                $(field).after($error);
                
                // Добавляем классы через jQuery
                $(field).addClass('search__input--invalid');
                searchForm.addClass('invalid'); // ← searchForm — jQuery-объект, всё ок!
            }
            ...
            function validationForm() {
                ...
                // Функция проверки одной строки
                function isValidLine(str) {
                    if (!str) return false;
            
                    const hasCyrillic = /[а-яА-ЯёЁ]/u.test(str);
                    const hasNonCyrillic = /[^а-яА-ЯёЁ\s]/u.test(str); // всё, кроме кириллицы и пробелов
                    // Но нам нужно: если есть кириллица — то НЕ должно быть латиницы/других букв
                    // Лучше так:
            
                    const hasLatin = /[a-zA-Z]/.test(str);
                    // Смешивание = есть и кириллица, и латиница
            ...
            function handleFileUpload(file) {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                       ...
                       resultsContainer.html(`
                             <div class="search-loading">
                                <img src="https://megaatom.com/wp-content/themes/gorn-2/images/loading.gif" alt="Загрузка..."  style="width: 60px; height: 60px;">
                            </div>
                        `);
                    },
          ...
          // Единственная точка входа — клик по документу
            document.addEventListener('click', function(e) {
                // Проверяем, кликнули ли по заголовку сортировки
                const header = e.target.closest('.sortable');
                if (!header) return;
            
                e.preventDefault();
            
                // Определяем таблицу, колонку и текущее направление
                ...
                // ← НЕ вызываем initQuantityInputs() и initCartButtons() здесь
                // ← Они должны быть привязаны к другим событиям или работать через делегирование
            });
            // Инициализация полей ввода количества
                ...
                function initQuantityInputs() {
                        $('.quantity-input').each(function() {
                            ...
                            // === ПОДСВЕТКА ЦЕНЫ ПО УМОЛЧАНИЮ (при загрузке) ===
                            if ($multiItems.length > 0) {
                                let bestPrice = null;
                                let bestItem = null;
                    
                                // Сбрасываем стиль у всех
                        ...
                        // Навешиваем обработчики на изменения
                        $('.quantity-input').off('change input').on('change input', function() {
                            // Повторяем ту же логику при изменении
                            const $input = $(this);
                            const minQ = parseInt($input.attr('min')) || 1;
                            const maxQ = parseInt($input.attr('max')) || 1;
                            let quantity = parseInt($input.val()) || minQ;
            ...
            // Инициализация кнопок "Добавить в корзину"
             function initCartButtons() {
                jQuery('.add-to-cart-btn').off('click').on('click', function () {
...
document.addEventListener('click', function (e) {
    ...
    const headers = Array.from(table.querySelectorAll('thead th'))
        .slice(0, -1) // убираем последнюю колонку "В корзину"
    ...
    table.querySelectorAll('tbody tr').forEach(tr => {
        const row = [];
        let multiItems = null; // для хранения данных из multiPrice
        
        // Проходим по всем ячейкам, кроме последней
        tr.querySelectorAll('td').forEach((td, index) => {
            if (index === headers.length) return; // пропускаем последнюю колонку

            if (td.classList.contains('price-rub')) {
                // Сохраняем multiPrice элементы для использования в колонке суммы
                multiItems = td.querySelectorAll('.multiPrice__item');
                
                if (multiItems.length > 0) {
                    // Формируем цены для экспорта
                ...
                // Формируем строки суммы именно в нужном формате
    ...
    // Включаем перенос текста в ячейках (для отображения в столбик)
    ...
    // Запускаем функцию ожидания jQuery
    waitForJQuery();
